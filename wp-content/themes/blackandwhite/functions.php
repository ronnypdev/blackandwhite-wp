...
if ( ! function_exists( 'blackandwhite_setup' ) ) :
	...
	function blackandwhite_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'blackandwhite' ),
            'services-menu' => esc_html__( 'Services', 'blackandwhite' ),
 ...
 */
function blackandwhite_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'blackandwhite_content_width', 640 );
}
add_action( 'after_setup_theme', 'blackandwhite_content_width', 0 );
...
function blackandwhite_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'blackandwhite' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'blackandwhite' ),
...
function blackandwhite_scripts() {
	...
	wp_enqueue_script( 'blackandwhite-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'blackandwhite-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

    wp_enqueue_script( 'jquery_slim', 'https://code.jquery.com/jquery-3.2.1.slim.min.js');
	wp_enqueue_script('propper_js', 'https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js');
